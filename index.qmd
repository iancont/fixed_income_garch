...
La gráfica de retornos diarios del IRP-GOBIX muestra baja volatilidad entre 2015 y 2022, con retornos generalmente dentro de un rango de ±50 puntos básicos. En varios periodos, se observa un fuerte incremento en la volatilidad, alcanzando picos de 200 básicos en el ciclo de tasas de 2018. Este comportamiento refleja un fenómeno de **"volatility clustering"**, donde períodos de baja volatilidad son seguidos por fases de alta volatilidad. Los choques recientes pueden estar relacionados con factores externos como cambios en tasas de interés o incertidumbre macroeconómica. Esto subraya la importancia de aplicar modelos ARMA-GARCH para capturar la dinámica y prever el comportamiento futuro del mercado de deuda pública dominicana. Estos fenómenos de volatility clustering se visualizan solapados con cambios de tendencia en los ciclos de tasas. 

## Análisis descriptivo de la muestra {#sec-estadistica-descriptiva}

<!-- {{< embed notebooks/data-screening.qmd#table-descriptive-stats-return >}} -->

La tabla de análisis descriptivo actualizada de los retornos muestra información clave sobre la distribución de la serie temporal de rendimientos. Con 1,941 observaciones, la **media** de los retornos es de $1.353251$ lo que indica un rendimiento positivo promedio. Sin embargo, la **mediana** de $-0.3180444$ sugiere que la distribución está ligeramente sesgada hacia valores negativos. La **desviación estándar** de $23.502911$ revela una amplia dispersión alrededor de la media, lo que refleja una elevada volatilidad en los rendimientos, significativamente mayor en comparación con la media.

El **valor de asimetría** ($0.70527$) sugiere una leve **asimetría positiva**, lo que indica que la serie tiene una ligera inclinación hacia valores extremos en la cola derecha. La **kurtosis** de **7.587995** sugiere que la distribución es **leptocúrtica**, lo que implica una mayor concentración de valores alrededor de la media y colas más pesadas que una distribución normal, lo que es común en datos financieros que presentan eventos extremos.

El **test de Jarque-Bera**, con un valor de $4,789.532331$ confirma que la serie no sigue una distribución normal, validando la presencia de asimetría y colas pesadas en la distribución de los rendimientos. Esto refuerza la necesidad de utilizar modelos que puedan capturar adecuadamente estos comportamientos no lineales y la alta volatilidad del mercado.
...
El estadístico de Kolmogorov-Smirnov es de $0.0286$, con un p-valor de $0.0827$, lo que indica que, aunque la distribución t de Student captura gran parte de los retornos, no es un ajuste perfecto. Este resultado sugiere que existen discrepancias entre la distribución teórica y los datos reales, particularmente en las colas de la distribución, donde se observan valores extremos. 

## Test de estacionariedad/robustez {#sec-estacionariedad}

El test de Dickey-Fuller aumentado (ADF) se utilizó para evaluar la estacionariedad de la serie de retornos, un requisito fundamental para la estimación de modelos ARMA y GARCH. Los resultados obtenidos muestran un **ADF Statistic** de $-13.1741$ y un **p-valor** de $1.2333 \times 10^{-24}$, lo que permite rechazar la hipótesis nula de raíz unitaria al nivel de significancia del 5%. Esto confirma que la serie de retornos es estacionaria, es decir, no presenta una tendencia significativa y sus fluctuaciones se distribuyen alrededor de una media constante a lo largo del tiempo. Estos resultados son consistentes con el comportamiento típico de las series de retornos financieros, lo que habilita la estimación adecuada de modelos ARMA-GARCH sobre la serie.
