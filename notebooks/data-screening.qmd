...
ax.plot(df.index, df['price_return'], label='Retorno Precio Diario')
ax.set_ylabel('Price Return')
ax.set_xlim([df.index.min(), df.index.max()])
ax.yaxis.set_major_formatter(PercentFormatter(xmax=1))
ax.legend()
plt.title('Serie de retornos diarios IRP-GOBIX')
plt.show()
```
...
plt.title('Distribution Fitting to Price Returns')
...
plt.title("Q-Q Plot of Price Return against Student's t-distribution")
plt.grid(True)
plt.show()
```
...
# | fig-cap: Comparación de valores predichos y actuales en el backtesting.
plt.figure(figsize=(10, 6))
plt.plot(R_test.index, R_test,
         label='Actual Moving Std', color='blue')
plt.plot(forecasted_std.index,
         forecasted_std, label='Forecasted Moving Std', color='red', linestyle='--')

plt.title('Actual vs Forecasted Moving Std - Zero-GARCH Model')
plt.legend()
plt.show()
```
